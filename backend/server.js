...
const directSquareEnv = String(process.env.SQUARE_ENV || "sandbox").trim().toLowerCase() === "production"
...
function getReceiptCopy(lang, orderId){
  const language = String(lang || "en").toLowerCase();
  if(language === "fr"){
    return {
      headerText: "Recu",
      introText: "Merci pour votre commande chez Power Poly Supplies. Nous la preparons avec soin. Voici votre recu.",
      subject: `Merci pour votre commande ! Power Poly Supplies - ${orderId}`
    };
  }
  if(language === "es"){
    return {
      headerText: "Recibo",
      introText: "Gracias por tu pedido en Power Poly Supplies. Ya estamos preparando tu pedido. Aqui esta tu recibo.",
      subject: `Gracias por tu pedido! Recibo de Power Poly Supplies - ${orderId}`
    };
  }
  if(language === "hi"){
    return {
      headerText: "Raseed",
      introText: "Power Poly Supplies mein aapke order ke liye dhanyavad. Hum aapka order tayyar kar rahe hain. Yah aapki raseed hai.",
      subject: `Aapke order ke liye dhanyavad! Power Poly Supplies raseed - ${orderId}`
    };
  }
  if(language === "ta"){
    return {
      headerText: "Raseethu",
      introText: "Power Poly Supplies-il ungal orderukku nandri. Ungal orderai thayarittu kondirukkirom. Itho ungal raseethu.",
      subject: `Ungal orderukku nandri! Power Poly Supplies raseethu - ${orderId}`
    };
  }
  return {
    headerText: "Receipt",
    introText: "Thanks for choosing Power Poly Supplies. We are getting your order ready now. Here is your receipt.",
    subject: `Thanks for your order! Power Poly Supplies receipt - ${orderId}`
  };
}
...
function requireDirectSquareConfigured(res){
  if(!directSquareClient || !directSquareLocationId){
    res.status(500).json({
      ok:false,
      message:"Square payments are not configured. Set SQUARE_ACCESS_TOKEN and SQUARE_LOCATION_ID on the backend."
    });
    return false;
  }
  return true;
...
app.post("/api/pay", (req,res,next)=> enforceRecaptcha(req, res, next, { action:"card_payment" }), async (req,res)=>{
  ...
  if(!token){
    return res.status(400).json({ ok:false, message:"Payment token is required." });
  }
  if(!Number.isInteger(amount) || amount <= 0){
    return res.status(400).json({ ok:false, message:"Amount must be a positive integer in cents." });
  }
  if(currency !== "CAD"){
    return res.status(400).json({ ok:false, message:"Only CAD payments are supported by this route." });
  }

  try{
    const result = await directSquareClient.payments.create({
      sourceId: token,
      idempotencyKey: crypto.randomUUID(),
      locationId: directSquareLocationId,
      amountMoney: {
    ...
    console.error("Square /api/pay error", err);
    const summary = summarizeSquareError(err);
    return res.status(summary.statusCode || 500).json({
      ok:false,
      message: summary.message || "Payment failed. Please try again.",
      errors: summary.errors || []
    });
  }
});
...
async function maybeSendPaidReceipt({ orderRow, orderId, status, statusWas }){
  ...
  if(emailConfigured && (customerEmail || adminEmail)){
    ...
    const receiptCopy = getReceiptCopy(lang, orderId);
    const headerText = lang === "fr"
      ? "Re̤u"
      : lang === "es"
        ? "Recibo"
    ...
    const receiptHtml = buildReceiptHtml({
      ...
      currency,
      taxLabel,
      headerText: receiptCopy.headerText,
      introText: receiptCopy.introText,
      logoUrl: getPublicLogoUrl(),
      logoCid: "pps-logo",
      logoExists,
      language: lang
          ...
          : lang === "es"
            ...
            : lang === "hi"
              ...
              : lang === "ta"
                ...
                : `Thanks for your order! Power Poly Supplies receipt - ${orderId}`;
        await sendEmailAny({ from: getSenderAddress(), to: customerEmail, subject: receiptCopy.subject, html: receiptHtml, attachments });
      }
      if(adminEmail){
        await sendEmailAny({
          from: getSenderAddress(),
