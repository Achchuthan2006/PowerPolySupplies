...
function provinceLabel(code){
  ...
  const namesFr = {
    ON:"Ontario", NS:"Nouvelle-Écosse", NB:"Nouveau-Brunswick", NL:"Terre-Neuve-et-Labrador", PE:"Île-du-Prince-Édouard",
    AB:"Alberta", BC:"Colombie-Britannique", SK:"Saskatchewan", MB:"Manitoba", QC:"Québec",
...
document.getElementById("payOnline").addEventListener("click", async ()=>{
  ...
  const shippingLine = shippingCents > 0 ? [{
    ...
    if(res.ok && data?.url){
      ...
      ? "Payment service took too long to respond. Render may still be waking up, so please try again in 30-60 seconds."
